...
% rou_bcc=1.7e14; % ferrite nuclei density [m-3]
rou_bcc=2.54e14;
% Rate=10;  % cooling rate [K/s]
Rate=0.4;  % cooling rate [K/s]
u=0.13;   % to adjust the growth velocity, it is for diffusion controlled model
...
%%%%%%%%%%%%% phase diagram in para-equilibrium condition Nov 21, 2016
a_eq(5,:)=[0.0000166618 -0.04012 24.12059];
b_eq(5,:)=[-0.0000996012 0.11565];

%%% 2-nd polynomial fits for kafang as a function of T [K]
% kafang_p=[0.0025 -5.365 2979.3]; % kafang(T)=a*T^2+b*T+c [J/(mol*mol%)], first estimate
kafang_p=[0 -1.1186 1296.2]; % kafang(T)=a*T^2+b*T+c [J/(mol*mol%)], improved on 14/03/2017
...
dT=1; dt=0.8*dT/Rate; % T and time step of calculation in seconds
...
%         N_p(m,19)=T(i)-(TR+0.5*(m_fcc*(N_p(m,15)-wCR_fcc)+m_bcc*(N_p(m,17)-wCR_bcc))); % local undercooling [K], Pina's method
    ...
    if CNT_flag==0
        ...
        if Timer(i)==0
           ...
           [Nt_cnt(i) EnergyB(i) Freq(i) Scailing(i)]=CNT(T(i),F(i-1),length(site),deltaGV(i),Timer(i),dt,Rate);% nucleation rate based on Classical Nucleation Theory [s-1]
        end
        %%%% accumulation
        if i==1
            Nreal(i)=Nt_cnt(i)*dt;
            Neff(i)=fix(Nt_cnt(i)*dt);
            Nrest(i)=Nreal(i)-Neff(i);
            N_eff_cnt(i)=Neff(i);
        else
            Nreal(i)=Nt_cnt(i)*dt+Nrest(i-1);
            Neff(i)=fix(Nreal(i));
            Nrest(i)=Nreal(i)-Neff(i);
      ...
      if F(i-1)<=1.01*F_eq(i)
         if t>=N_p(l,6) && N_p(l,6)>0  % && N_p(l,13)==0
             for r=1:length(N_PR{l}(:,1))
                 ...
                 [DiffInfo{l}(i,r,:)]=Mixmode_diffusion_profile1(xC_F_eq(i),xC_A_eq(i),N_p(l,34), ...
                     D_C(i)*1e-12,Mobility(i),Kafang(i),N_p(l,7),length(N_PR{l}(:,1)),N_p(l,32));% mix-mode
                 HardFlag{l}(i,r)=0; % hard flag
                 [DiffInfoD{l}(i,r,:)]=Diffusion_control_profile(xC_F_eq(i),xC_A_eq(i),Comp_m(1), ...
                     D_C(i)*1e-12,Mobility(i),Kafang(i),N_pD(l,7),length(N_PR{l}(:,1)),N_p(l,32)); % diffusion-controlled
                 ...
                 % mol%, mol%, um, um, mol%, mol%, 1or0;
                    ...
                    if DiffInfo{l}(i,r,2)<DiffInfo{l}(i-1,r,2)
                       [DiffInfo{l}(i,r,:)]=Mixmode_diffusion_profile1(xC_F_eq(i),xC_A_eq(i), ...
                           N_p(l,34),D_C(i)*1e-12,Mobility(i),Kafang(i),N_p(l,7),length(N_PR{l}(:,1)),N_p(l,32));% mix-mode
                    else
                 ...
                 for aa=1:length(A)
                     ...
                     for bb=1:length(A_PR{aa})
                         ...
                         if isempty(dis_ferrite) && N_p(A_PR{aa}(bb),31)~=0
                             ...
                             bb_dis=bb_dis+1;
                                dis_ferrite(bb_dis)=min([d(A_PR{aa}(bb),cc) d_MO(A_PR{aa}(bb),cc) ...
                                    d_MXY(A_PR{aa}(bb),cc) d_MXZ(A_PR{aa}(bb),cc) d_MYZ(A_PR{aa}(bb),cc)]);
                                if (bb_dis>1 && dis_ferrite(bb_dis)<dis_ferrite(bb_dis-1)) || bb_dis==1
...
%            N_p(m,8)=((3*(1-exp(-4/3*pi*N_p(m,7)^3/N_p(m,29)))*N_p(m,29))/(4*pi))^(1/3); % correctiong of extended volume based on Avrami approach
             N_p(m,8)=((3*tanh(4/3*pi*N_p(m,7)^3/(4/3*pi*N_p(m,32)^3))*(4/3*pi*N_p(m,32)^3))/(4*pi))^(1/3); % correction with tanh(xe)          
           if N_p(m,8)<Nucleated{i}(mm,8) || N_p(m,8)>1.05*Nucleated{i}(mm,8)
               if ismember(m,Nucleated{i-2}(:,1)) && ismember(m,Nucleated{i-1}(:,1))
                   N_p(m,8)=Nucleated{i-1}(mm,8)+1/2*((Nucleated{i-1}(mm,8)-Nucleated{i-2}(mm,8))/dt)*dt;
               else
                   N_p(m,8)=N_p(m,8)+v_t(i,m)*dt;
               end
           end
        ...
        N_p(b,12)=0; %ratio of volume fraction to equilibrium volume fraction predicted by phase diagram
            ...
            if ~isempty(find(minD_d==minD))
             ...
             if V_Fsurrounding==0 && N_p(b,31)~=0
                 N_p(b,32)=min([N_p(b,32) 2*((3*Lb^3/N0)/(4*pi))^(1/3)-N_p(N_p(b,31),7)]);
             else
                 N_p(b,32)=min([N_p(b,32) 2*(3*(Lb^3/N0-V_Fsurrounding)/(4*pi))^(1/3)]);
             end
...
%              N_p(b,32)=(3*(Lb^3/N0)/(4*pi))^(1/3); % average growing d [um]
               N_p(b,32)=mean(N_PR{b}(:,3)); % average vertice and austenite center distance [um]
...
% correction is needed for >impingement4
CorrFlag=1;
if CorrFlag==1
    SC=max(F_eq)/(max(F_eq)*tanh(sum(4/3*pi*(Nucleated{end}(:,7)).^3)./Lb^3));
    Impinge4Start=[];
    ImpingeStart=[];
    for i=1:length(Timer)
       ...
       if ~isempty(Nucleated{i})
         while mm<=length(Nucleated{i}(:,1)) %calculate the mean value and the volume fraction
            m=Nucleated{i}(mm,1);
            volume=volume+4/3*pi*Nucleated{i}(mm,8)^3; %used for the impingement34 function
            mm=mm+1;
         end
       end
        F(i)=volume/Lb^3; %volume fraction
        if ~isempty(Nucleated{i})
            if ~all(Nucleated{i}(:,13)==0)
               F_corr(i)=SC*F_eq(i)*tanh(sum(4/3*pi*(Nucleated{i}(:,7)).^3)./Lb^3);
               if F_corr(i)>F_eq(i)
                   F_corr(i)=F_eq(i)-eps;
               end
               if F_corr(i)>F(i)
                   F(i)=F_corr(i);
                   ExtraV(i)=F_corr(i)*Lb^3-sum(4/3*pi*(Nucleated{i}(:,8)).^3);
                   Nucleated{i}(:,8)=(3*(4/3*pi*Nucleated{i}(:,8).^3+ExtraV(i)/length(Nucleated{i}(:,1)))./(4*pi)).^(1/3);
               end
            end
        end
        if ~isempty(Nucleated{i})
             delta(i)=mean(Nucleated{i}(:,8));
             R_sd(i)=std(Nucleated{i}(:,8));
             Ve(i)=sum(4/3*pi*Nucleated{i}(:,7).^3);
             if ~isempty(Nucleated{i-1}) && length(Nucleated{i}(:,1))>2
                 for j=1:length(Nucleated{i-1}(:,1))
                    if Nucleated{i-1}(j,13)==0 && Nucleated{i}(j,13)==1 && isempty(Impinge4Start)
                        Impinge4Start=i;
                    else if Nucleated{i-1}(j,26)==0 && Nucleated{i}(j,26)==1 && isempty(ImpingeStart)
                         ImpingeStart=i;
                        end
                    end
                 end
             end
             if ~isempty(Impinge4Start)
                 Impinge4Start=min(Impinge4Start);
             end
             if ~isempty(ImpingeStart)
                 ImpingeStart=min(ImpingeStart);
             end
        else
            delta(i)=0;
            R_sd(i)=0;
...
subplot(2,3,6);
plot(Timer,HardFlag{TrackNo}(:,1),'ro-');
xlabel('Time (s)');
ylabel('Flag of hard impingement');
set(gca,'fontsize',12);
set(gca,'linewidth',2);
for b=1:length(Timer)
    if ~isempty(Nucleated{b}) && ismember([TrackNo],Nucleated{b})
